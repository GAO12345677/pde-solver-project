...
try:
    ...
    result = resp.json()
    print(f"   算法: {result.get('algorithm_key')}")
    print(f"   计算时间: {result.get('computation_time_ms')} ms")
    print(f"   数据点数: {len(result.get('solution', []))}")
except Exception as e:
    print(f"❌ 1D 热传导方程求解失败: {e}")

# 7. 测试 2D Poisson 方程求解
print("\n7. 测试 2D Poisson 方程求解")
payload = {
...
try:
    resp = requests.post(f"{base_url}/solve_equation", json=payload, timeout=30)
    print(f"✅ 2D Poisson 方程求解 - 状态码: {resp.status_code}")
    result = resp.json()
    print(f"   算法: {result.get('algorithm_key')}")
    print(f"   计算时间: {result.get('computation_time_ms')} ms")
    print(f"   数据点数: {len(result.get('solution', []))}")
except Exception as e:
    print(f"❌ 2D Poisson 方程求解失败: {e}")
